...
function isMilkInFridge(fridgeContents) {
  ...
  return fridgeContents.includes("milk") ? true : false;
}
runTest(
...
//   const mystery = devs.map((x) => {
//     if (x.name.includes("ch")) return (x.isCool = true);
//   });
// }
// console.log(mystery);
...
function affordableSnacks(snacks, money) {
  ...
  const result = snacks.filter((xx) => {
    return xx.cost <= money;
  });
}
...
//   const mystery = strings.map((string) => {
...
function isAllAboveSix(films) {
  ...
  const aboveThreshold = films.every((film) => {
    return film.rating >= 6;
  });
...
function namesOfStaffOver28(staff) {
  ...
  let names = [];
  const result = staff.filter((xx) => xx.age > 28);
  result.forEach((person) => names.push(person.name));
